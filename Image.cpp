...
#include "Image.h"
#include <algorithm> // Add this include directive at the top
...
#include <cmath> // For using std::floor, std::round
#include <cstdio> // For using fprintf

bool MyImage::HSVSegmentation(int h1, int h2)
{
	// Ensure hue thresholds are within the valid range
	if (h1 < 0 || h1 > 360 || h2 < 0 || h2 > 360 || h1 >= h2) {
		fprintf(stderr, "Error: Invalid hue thresholds.\n");
		return false;
	}
	...
	for (int i = 0; i < Width * Height; i++) {
		// Extract RGB values
		unsigned char r = Data[3 * i + 2]; // Red
		unsigned char g = Data[3 * i + 1]; // Green
		unsigned char b = Data[3 * i];     // Blue
		...
		float cMax = std::fmax(rNorm, std::fmax(gNorm, bNorm));
		float cMin = std::fmin(rNorm, std::fmin(gNorm, bNorm));
		...
		if (delta != 0) {
			if (cMax == rNorm) {
				h = 60.0f * std::fmod(((gNorm - bNorm) / delta), 6);
			}
			else if (cMax == gNorm) {
				h = 60.0f * (((bNorm - rNorm) / delta) + 2);
			}
			else if (cMax == bNorm) {
				h = 60.0f * (((rNorm - gNorm) / delta) + 4);
			}
		}
		...
		// Check if the hue is within the specified range (considering wrap-around)
		if ((h1 < h2 && (h < h1 || h > h2)) || (h1 >= h2 && (h < h1 && h > h2))) {
			// Convert to grayscale
			unsigned char gray = static_cast<unsigned char>(0.299 * r + 0.587 * g + 0.114 * b);
			Data[3 * i] = gray;     // Blue
			Data[3 * i + 1] = gray; // Green
			Data[3 * i + 2] = gray; // Red
		}
		// If within the hue range, keep the original color (no change needed)
	}

	return true; // Segmentation complete
}
