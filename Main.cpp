	...
	int       nCmdShow)
{
	...
	// Default values
	char imageName[MAX_PATH] = "roses_image_512x512.rgb";
	int width = 512;  // Fixed width
	int height = 512; // Fixed height
	int optionSelect = 320;
	int optionType = 359;
	...
	// Print the default inputs
	printf("Default Image Name: %s\n", imageName);
	printf("Default Width: %d\n", width);
	printf("Default Height: %d\n", height);
	printf("Default Option Select: %d\n", optionSelect);
	printf("Default Option Type: %d\n", optionType);

	// Parse command line arguments
	char* token = strtok(lpCmdLine, " ");
	if (token != NULL) {
		// First argument is the image name
		strcpy_s(imageName, token);
	}

	token = strtok(NULL, " ");
	if (token != NULL) {
		// Second argument is the optionSelect
		optionSelect = atoi(token);
	}

	token = strtok(NULL, " ");
	if (token != NULL) {
		// Third argument is the optionType
		optionType = atoi(token);
	}

	// Print the inputs (after parsing command line)
	printf("Image Name: %s\n", imageName);
	printf("Width: %d\n", width);
	printf("Height: %d\n", height);
	...
	// Set up the image with the parsed or default dimensions
	inImage.setWidth(width);
	inImage.setHeight(height);
	inImage.setImagePath(imageName);
	inImage.ReadImage();

	if (!inImage.HSVSegmentation(optionSelect, optionType)) {
		std::cerr << "Failed to perform HSV segmentation." << std::endl;
		return -1;
	}
